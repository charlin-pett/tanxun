# 📋 项目日志

## 项目信息
- **项目名称：** 探寻
- **项目路径：** D:\探寻
- **创建日期：** 2026-07-07
- **技术栈：** Next.js 16 + TypeScript + Tailwind CSS + next-intl 4.x
- **部署平台：** Vercel（目标）
- **语言路线：** 中文 → 英文 → 俄语

---

## 日志条目

### 2026-07-07 — Phase 1：项目脚手架搭建完成

#### ✅ 已完成
1. **项目初始化**
   - 创建 D:\探寻 项目目录
   - 用 `create-next-app` 初始化 Next.js 16 项目（TypeScript + App Router + Tailwind）
   - 安装 `next-intl` 4.x 国际化依赖

2. **国际化架构搭建**
   - `src/i18n/request.ts` — next-intl 请求配置，支持多语言消息加载
   - `src/proxy.ts` — 语言检测中间件（Next.js 16 改用 proxy 替代 middleware）
   - `src/i18n/navigation.ts` — 导航工具封装（`Link`/`redirect`/`usePathname`/`useRouter`）
   - `messages/zh-CN.json` — 中文语言包（site/nav/home/dream/bazi/knowledge/hexagram/common 全部 UI 文案）

3. **项目目录结构**
   ```
   D:\探寻/
   ├── src/              ← 所有源码
   │   ├── app/          ← 页面路由
   │   ├── components/   ← 共用组件
   │   ├── data/         ← 知识库数据
   │   ├── i18n/         ← 国际化配置
   │   ├── lib/          ← 工具函数
   │   ├── types/        ← 类型定义
   │   └── proxy.ts      ← 语言检测中间件
   ├── messages/         ← 国际化语言包
   └── public/           ← 静态资源
   ```

4. **页面路由全部就绪**
   - `/` → 重定向到 `/zh-CN/`
   - `/zh-CN/` → 首页（Hero + 四大功能卡片 + 每日运势占位）
   - `/zh-CN/dream` → 解梦页（占位）
   - `/zh-CN/bazi` → 八字排盘页（占位）
   - `/zh-CN/hexagram` → 六十四卦列表 + 详情页
   - `/zh-CN/knowledge` → 玄学百科页（占位）

5. **UI 组件**
   - **Header** — 导航栏 + Logo（太极图标 CSS 绘制）+ 语言切换下拉框
   - **Footer** — 版权声明 + 免责声明（玄学类网站必填）
   - **FeatureCard** — 首页功能卡片（梦/八字/卦/书 四个 SVG 图标）
   - **LocaleSwitcher** — 语言选择器（中/英/俄，使用 `startTransition` 不阻塞 UI）
   - **国风设计系统** — 宣纸底 `#f5efe0`、墨色/朱红/金色配色

6. **六十四卦数据库**
   - 录入全部 64 卦数据（卦名、卦符、卦辞、六爻爻辞+释义、象传、现代释义）
   - 卦象列表页：8x8 网格视图，显示卦符+卦名
   - 卦象详情页：卦符+卦名+卦辞+爻辞+象传+现代释义+前后卦导航
   - 非法卦序（0, 65等）自动返回 404

#### 📝 踩坑记录
- Next.js 16 中 `middleware.ts` 已弃用，需改为 `proxy.ts`
- next-intl v4 中 `createSharedPathnamesNavigation` → `createNavigation`
- `getRequestConfig` 必须返回 `locale` 字段（字符串类型）
- npm 不能使用中文项目名，用 `tanxun` 作为代码项目名
- **所有源码文件必须放在 `src/` 目录下**（`@/` 映射指向 `./src/`），包括 data/ types/ lib/ engine/ prompts/

#### 🔜 下一步计划
- **Phase 3:** 八字排盘算法引擎（排四柱 + 十神 + 大运 + 流年）
- **Phase 4:** 解梦功能（AI 接入）

### 2026-07-07 — Phase 2 & Phase 3：六十四卦 + 八字排盘完成

#### ✅ 六十四卦数据库（Phase 2）
- 录入全部 64 卦数据（卦名、卦符、卦辞、六爻爻辞+释义、象传、现代释义、分类）
- 卦象列表页：8×8 网格视图，每格显示卦符+卦名，点击进入详情
- 卦象详情页：卦符区+卦辞+象传+六爻逐条展示+现代释义+前后卦导航
- 非法卦序自动返回 404

#### ✅ 八字排盘算法引擎（Phase 3）
- **`src/engine/types.ts`** — 核心数据结构（天干/地支/五行/六十甲子/四柱/大运/流年等类型定义）
- **`src/engine/ganzhi.ts`** — 底层计算函数（闰年判断、公历转干支、五虎遁/五鼠遁、时辰索引、月索引等）
- **`src/engine/bazi.ts`** — 排盘核心（排四柱、十神判定、五行统计、大运顺逆、流年计算）
- **`src/engine/labels.ts`** — 中文标签转换（引擎输出索引→中文文字）

#### ✅ 八字排盘UI（Phase 3）
- **BaziForm** — 全功能表单（年份/月份/日期/时间/性别输入，支持时辰选择和精确时间两种模式）
- **BaziResultView** — 命盘展示（四柱八字表格、日主标注、十神显示、五行柱状图、大运列表、流年）
- 所有计算在浏览器端完成，无需 API 调用

#### 📝 踩坑记录
- `ReturnType<typeof calcBaZi>['daYun']` 在客户端组件中使用需要确保类型正确
- 排盘算法中使用的是近似节气月，后续如需精确需根据立春日期调整
- 起运年龄简化为3岁（精确计算需根据出生日到节气日期的天数÷3）

### 2026-07-07 — Phase 4：星座 + AI命理报告全链路完成

#### ✅ 星座计算
- **`src/engine/zodiac.ts`** — 纯算法，根据公历月日返回星座索引 (0-11)，语言无关
- **`src/data/zodiac/zh-CN.json`** — 12 星座完整数据（性格描述、优点缺点、爱情事业、幸运信息）
- **`ZodiacCard`** — 星座展示卡片（符号、元素、关键词、优缺点、性格简述）
  - 根据出生月日自动计算，展示在命盘上方

#### ✅ AI命理报告系统
- **`src/engine/prompts/reading-zh-CN.ts`** — AI 提示词模板（中文 system prompt + user prompt，英文备用）
- **`src/app/api/reading/route.ts`** — 命理报告生成 API
  - **双模型策略：** DeepSeek（国内） / Claude（海外） / Mock（无需API Key）
  - 接收八字排盘数据 + 星座信息，调用 AI 生成完整命理分析报告
  - 报告结构：星座性格 → 八字命格 → 五行分析 → 性格总论 → 运势提示 → 发展建议
- **`AiReading`** — AI 报告组件（四态）
  - `idle` 初始态 → 显示"开始测算命理"按钮
  - `loading` 加载中 → 太极旋转动画 + 加载提示
  - `done` 完成态 → 显示完整命理分析报告
  - `error` 错误态 → 显示错误信息 + 重试按钮
- **`.env.example`** — 环境变量说明文件（配置 API Key 后自动启用 AI）

#### ✅ 整合展示
- **BaziResultView** 升级为完整三件套：
  1. 🏷 星座卡片（上方）
  2. ☯ 八字命盘 + 五行统计 + 大运 + 流年（中间）
  3. 🤖 AI 命理分析报告（下方，含星座+八字综合分析）

### 2026-07-07 — Phase 5：过渡动画完成

#### ✅ 全屏过渡动画
- **`BaziTransition`** — 全屏遮罩动画组件，三个递进阶段：
  - 0-1.5s：太极旋转 + 八卦符号（䷀䷁䷂䷃…）在四周飘散
  - 1.5-3.2s：五行文字金木水火土以对应颜色轮转（金色/翠绿/深蓝/朱红/檀木）
  - 3.2-4.7s：星座符号（♈♉♊♋♌♍…）星空气氛
  - 4.7-5.5s：收尾淡出缩放
- 底部渐变进度条 + 动态阶段文字
- 动画结束后自动显示完整结果

#### ✅ 三屏流程重构
- **BaziForm** 改为三屏状态机：`form → transition → result`
  1. 用户填写表单 → 点击"排盘测算"
  2. 全屏过渡动画 (5.5s)
  3. 自动显示星座卡片 + 八字命盘 + AI 命理报告（无需再点击）
- **AiReading** 支持 `autoFetch` 模式，挂载后自动生成报告
- 新增 `globals.css` 中 `animate-spin-slow` 自定义动画

#### 🔜 后续规划
- **配置 API Key** — 在 `.env.local` 中设置 `DEEPSEEK_API_KEY`
- **解梦功能（AI接入）**
- **英文/俄语版本**
- **升级AI命理报告（配置API Key）**

### 2026-07-07 — Phase 6：玄学百科知识库完成

#### ✅ 百科数据（15篇高质量文章）
| 分类 | 文章数 | 内容 |
|------|--------|------|
| 周易 | 4篇 | 易经入门、八卦详解、六十四卦结构、易经生活智慧 |
| 五行 | 2篇 | 五行学说、五行对应关系表 |
| 天干地支 | 2篇 | 干支系统、六十甲子纳音 |
| 奇门遁甲 | 2篇 | 奇门入门、排盘基础 |
| 风水 | 2篇 | 风水基础、房间布局指南 |
| 面相手相 | 3篇 | 面相入门、五官精讲、手相基础 |

#### ✅ 百科页面
- **百科首页** — 按分类展示，每类显示文章列表+摘要+标签
- **文章详情页** — 左侧分类目录+右侧正文，正文自动识别小标题加粗显示
- 支持前后篇导航，面包屑路径
- 分类配色系统（周易红、五行金、干支蓝、奇门紫、风水绿、面相粉）
- 不存在的文章自动返回 404

#### 📝 踩坑记录
- JSON 动态导入 (`import(\`path/${var}.json\`)`) 在 Turbopack 中可能只打包部分文件，需改为静态导入
- 中文引号在 JSON 中需要特别注意转义，建议用「」替代""避免问题

---

### 2026-07-07 — 当日总结

#### 今日完成（全天进度汇总）
1. **Phase 2:** 六十四卦完整数据 + 列表/详情页 ✅
2. **Phase 3:** 八字排盘引擎 + UI表单 + 命盘展示 ✅
3. **Phase 4:** 星座数据 + AI命理报告API（双模型架构） ✅
4. **Phase 5:** 全屏过渡动画（太极→五行→星座，5.5秒） ✅
5. **Phase 6:** 玄学百科知识库（6类15篇文章+分类/详情页） ✅
6. **部署:** 代码已推送到 GitHub，Vercel 已自动部署 ✅
7. **Phase 7:** AI 解梦功能 ✅
8. **多语言:** 英语/俄语/西班牙语 ✅

#### 待办
- ⏳ **配置 API Key** — 让 AI 报告和 解梦从 Mock 变成真实 AI 生成
- ⏳ **申请 Google AdSense** — 开始盈利
- ⏳ **英文/俄语版** — 国际化内容翻译
- ⏳ **git push** — 百科和过渡动画的新代码需推送到 GitHub 触发自动部署

#### 项目结构总览
```
D:\探寻/
├── LOG.md, MEMORIES.md, .env.example
├── messages/zh-CN.json          ← 中文UI语言包
├── src/app/
│   ├── [locale]/
│   │   ├── page.tsx             ← 🏠 首页
│   │   ├── bazi/page.tsx        ← 🔮 八字排盘（含过渡动画+星座+AI报告）
│   │   ├── hexagram/            ← ☯ 六十四卦（列表+详情）
│   │   ├── dream/page.tsx       ← 🌙 解梦（占位待开发）
│   │   └── knowledge/           ← 📖 玄学百科（6类15篇文章）
│   └── api/reading/             ← 🤖 AI命理报告API
├── src/components/              ← 13个组件
├── src/engine/                  ← 排盘算法引擎
├── src/data/                    ← 知识库数据
└── src/lib/                     ← 工具函数
```

#### 启动方式
```bash
cd /d/探寻 && npm run dev
# 访问 http://localhost:3000
```

### 2026-07-08 — Phase 7 & 8：解梦功能 + 多语言完成

#### ✅ AI 解梦功能
- **梦境分类引擎** — 7 大分类（动物/自然/人物/动作/物品/身体/场所），支持 4 种语言关键词匹配
- **解梦 API** — 复用双模型策略（DeepSeek/Claude/Mock），四种语言独立 prompt
- **解梦提示词模板** — 每种语言自成一体的系统 prompt 风格（中文半文半白、英文心理学、俄文神秘、西文温暖）
- **DreamInput 组件** — 纸笺风格输入框、实时分类标签、字数限制
- **DreamResult 组件** — 四板块解梦报告（古籍/心理/五行/建议）、相关卦象推荐
- **解梦历史记录** — localStorage 存储，最多 20 条，支持重新查看
- Mock 解梦：会根据梦境关键词（水/火/飞/坠落等）生成不同的解梦内容

#### ✅ 四种语言版本
| 语言 | locale | 状态 |
|------|--------|------|
| 🇨🇳 中文 | zh-CN | 完整（基准语言）|
| 🇬🇧 英语 | en | 完整翻译 |
| 🇷🇺 俄语 | ru | 完整翻译 |

#### ✅ 深度翻译工程（2026-07-08）

| 模块 | en | ru | es |
|------|:--:|:--:|:--:|
| UI 语言包（messages/） | ✅ | ✅ | ✅ |
| 星座数据（12星座） | ✅ 地道英式占星 | ✅ 俄式诗意风格 | ✅ 西语温暖热情 |
| 八字术语（天干地支十神）| ✅ | ✅ | ✅ |
| 知识库·周易文章 | ✅ | ✅ | ✅ |
| 知识库·五行文章 | ✅ | ✅ | ✅ |
| 知识库·干支文章 | ✅ | ✅ | ✅ |
| 知识库·奇门文章 | ✅ | ✅ | ✅ |
| 知识库·风水文章 | ✅ | ✅ | ✅ |
| 知识库·面相手相文章 | ✅ | ✅ | ✅ |
| AI 命理报告（prompt） | ✅ | ✅ | ✅ |
| AI 解梦（prompt） | ✅（Phase 7已完成） | ✅（Phase 7已完成） | ✅（Phase 7已完成） |
| 星座卡片组件 | ✅ 自动语言切换 | ✅ | ✅ |

#### ✅ AdSense 盈利准备（2026-07-08）
- **隐私政策页面** — 4种语言完整版（/zh-CN/privacy, /en/privacy 等）
- **关于我们页面** — 4种语言完整版（/zh-CN/about 等）
- **Footer** — 底部添加隐私政策和关于我们链接
- **AdSense 广告组件** — `AdSenseAd` 组件，审核通过后填入 slot ID 即可显示广告
- **AdSense 脚本组件** — `AdSenseScript` 自动加载 AdSense 脚本（环境变量控制）
- 各语言包添加 privacy / about 字段

#### 🔜 待办
- ⏳ 六十四卦翻译（64卦 × 4语言，量大，放最后）
- ⏳ Google AdSense 审核 + 配置环境变量

#### 📝 踩坑记录
- 动态 import dream 分类时需注意 Turbopack 打包范围
- 四种语言的 Mock 解梦内容需各自独立编写，不能用机器翻译敷衍

---

### 2026-07-08 — Phase 9：AdSense 盈利准备完成

#### ✅ 本日新增
1. **语言切换修复** — 发现切换语言时 placeholder/标签/百科分类名仍是中文
   - 修复：所有服务端组件显式从 `params` 获取 locale
   - 修复：DreamInput 所有文本标签按语言翻译（4种语言独立翻译表）
   - 修复：BaziForm 所有表单标签按语言翻译（含性别/按钮/错误提示等）
   - 修复：百科分类名音译（I-Ching, И-Цзин, Five Elements, Пять Элементов 等）
2. **隐私政策页面** — 4种语言完整版 ✅
3. **关于我们页面** — 4种语言完整版 ✅
4. **Footer** — 底部添加隐私政策和关于我们链接 ✅
5. **AdSense 广告组件** — 审核通过后填入 slot ID 即可用 ✅
6. **AdSense 脚本组件** — 环境变量控制加载 ✅
7. **代码已推送到 GitHub** ✅

#### 📝 新踩坑
- `getTranslations('namespace')` 无参数时在语言切换后不会刷新 → 必须改为 `getTranslations({ locale, namespace })`
- 客户端组件的硬编码中文文本需全部改为 `useLocale()` + 语言映射表
- 百科分类名不能写死在变量里，需改为按 locale 返回不同名称的函数

#### 🔜 明天可做
- ⏳ **六十四卦翻译**（64卦×3语言，大工程）
- ⏳ **每日运势/抽签**（新功能）
- ⏳ **姓名测试**（输入姓名分析五行）
- ⏳ Google AdSense 审核结果查看

---

### 2026-07-08（晚）— 发现问题：国内无法访问

#### 问题

---

### 2026-07-08（晚）— 发现问题：国内无法访问

#### 问题
- `.vercel.app` 域名在国内被屏蔽，国内用户打不开网站
- 从服务器端测试也确认全部超时（000 状态码）

#### 解决方案
- **购买自定义域名**（如 `tanxun.life`）绑定到 Vercel
- 用户将在阿里云购买域名，待到手后配置
- 用户反馈不需要数据库存储用户数据，保持现状（用户信息不留存服务器）

#### 今日工作收尾
- 所有代码已推送到 GitHub ✅
- Vercel 自动部署 ✅
- 服务器已关闭 ✅

---

### 2026-07-08 — Phase 10：六十四卦翻译 + AdSense 全线完成

#### ✅ 六十四卦三语翻译
- 64卦 × 3语言（en/ru/es）全量翻译
- 卦名、卦辞、象传完整翻译
- 爻辞保留汉字（篇幅原因）
- 文件大小：en(124KB) + ru(130KB) + es(124KB)

#### ✅ Google AdSense 全线跑通
- 审核通过 ✅
- 广告代码已部署到所有页面 ✅
- CMP 欧洲法规消息已配置 ✅
- 1小时内广告开始展示 ✅

#### ✅ 全面翻译修复（用户反馈切换语言仍有中文）
- BaziResultView：四柱八字/五行分布/大运/流年 全多语言
- BaziTransition：动画阶段文字 × 4语言
- AiReading：按钮/标题/加载文字 全多语言
- DreamResult：梦境内容/解梦分析/相关卦象 全多语言
- Hexagram详情页：第N卦统一格式
- Knowledge详情页：目录/正文/前后篇 全多语言

#### ✅ 每日一签（新增功能）
- 首页替换原来的占位符
- 每日根据日期确定唯一卦象（确定性随机）
- 显示卦名、卦符、卦辞
- 复制分享链接按钮
- 4语言支持（卦象数据按语言切换）
- 点击可跳转卦象详情页
- localStorage 记录，同一天不变

#### 🔜 明天可做
- ⏳ **姓名测试**（输入姓名分析五行三才）
- ⏳ **生肖运势**
- ⏳ **在线摇卦**
- ⏳ Google AdSense 收入查看

